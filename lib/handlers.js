    ...
    helpers.linkEncurtado(idNoticia, idProdutoMvc).then(function (dataEncurtador) {
      ...
      <html>
        <head>
          ...
          <meta property="og:description" content="${resNoticia.NomeFonte.trim()} | ${resNoticia.NomeProgramaSecao.trim()} | Veiculação: ${helpers.formatDate(resNoticia.DataHora.trim())} | Cadastro: ${helpers.formatDate(resNoticia.DataHoraCaptura.trim())}">
