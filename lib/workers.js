...
// Worker for checking expiry date and devarion (in case of expired file)
      ...
      for (var i = 0, iLen = files.length; i < iLen; i++){
        ...
        fs.stat(filePath, function(err, stats){
          if(!err){
            ...
            // If creation date is 24h older than now, devare
            ...
            console.log('Error at obtaining file stat');
          }
        })
      } 
    }else{
...
workers.loop = function(){
  setInterval(function(){
    console.log('Expiration loop started at:'+(new Date).toJSON());
    workers.checkExpiry();
  },1000 * 60);
};

workers.init = function (){
  console.log('Service inited at:'+(new Date).toJSON());
